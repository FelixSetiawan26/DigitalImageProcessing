...
begin
     ...
     begin
          for x:= 0 to image1.width-1 do
          begin
          SharpeningR := (tempR[x-1][y-1] * (-0.1)) + (tempR[x][y-1] * (-0.1)) + (tempR[x+1][y-1] * (-0.1))
                         + (tempR[x-1][y] * (-0.1)) + (tempR[x][y] * 1.8) + (tempR[x+1][y] * (-0.1))
                         + (tempR[x-1][y+1] * (-0.1)) + (tempR[x][y+1] * (-0.1)) + (tempR[x+1][y+1] * (-0.1));
          SharpeningG := (tempG[x-1][y-1] * (-0.1)) + (tempG[x][y-1] * (-0.1)) + (tempG[x+1][y-1] * (-0.1))
                         + (tempG[x-1][y] * (-0.1)) + (tempG[x][y] * 1.8) + (tempG[x+1][y] * (-0.1))
                         + (tempG[x-1][y+1] * (-0.1)) + (tempG[x][y+1] * (-0.1)) + (tempG[x+1][y+1] * (-0.1));
          SharpeningB := (tempB[x-1][y-1] * (-0.1)) + (tempB[x][y-1] * (-0.1)) + (tempB[x+1][y-1] * (-0.1))
                         + (tempB[x-1][y] * (-0.1)) + (tempB[x][y] * 1.8) + (tempB[x+1][y] * (-0.1))
                         + (tempB[x-1][y+1] * (-0.1)) + (tempB[x][y+1] * (-0.1)) + (tempB[x+1][y+1] * (-0.1));
          ...
          if SharpeningB<0 then
             tempB[x,y] := 0;
          image1.Canvas.Pixels[x,y] := RGB(tempR[x,y], tempG[x,y], tempB[x,y]);
          end;
     end;
end;
